...
function JavaParser.get_visibility(node, bufnr)
	-- vim.print(vim.treesitter.get_node_text(node:parent():parent(), opts.bufnr))
	for n, _ in node:parent():parent():iter_children() do
		local type = n:type()
		if type == "modifiers" then -- TODO: make this line language agnostic
			local modifiers = vim.treesitter.get_node_text(n, bufnr)
...
function JavaParser.parse_property(node, start_col, start_row, opts)
	local visibility = JavaParser.get_visibility(node, opts.bufnr)
